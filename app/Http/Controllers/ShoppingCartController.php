...
class ShoppingCartController extends Controller
{
    ...
    public function checkout()
    {
        ...
        // $checkAddress = false;
        $address = '';
        $getAddress = DB::select('SELECT DISTINCT `order`.EmailKH,`order`.DiaChi  FROM `order`');

        foreach ($getAddress as $item) {
            if($item->EmailKH == Auth::guard('customer')->user()->email) {
                $address = $item->DiaChi;
                break;
            };
        }
        return view('Home.checkout',compact('getAddress', 'address'));
    }
    ...
    public function orderAdd(Request $req)
    {
        ...
        if ($checkpayment == 'tructiep') {
            try {
                ...
                // Mail::to('easyrespect1@gmail.com')->send(new \App\Mail\MyTestMail($details));
